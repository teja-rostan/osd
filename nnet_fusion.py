...
def fusion_update(w_h, w_h2, w_o, rank, threshold):
    ...
    threshold = np.mean(abs(abs(fuser.complete(relations[0])) - (abs(w_h.get_value()))))
    c1 = abs(abs(fuser.complete(relations[0])) - (abs(w_h.get_value()))) < threshold
    c2 = abs(abs(fuser.complete(relations[1])) - (abs(w_h2.get_value()))) < threshold
    c3 = abs(abs(fuser.complete(relations[2])) - (abs(w_o.get_value()))) < threshold

    deg = 1 - (np.count_nonzero(c1) + np.count_nonzero(c2) + np.count_nonzero(c3)) / \
              float(len(c1.flat) + len(c2.flat) + len(c3.flat))
    w_h *= c1
    w_h2 *= c2
    w_o *= c3
    return w_h, w_h2, w_o, c1, c2, c3, deg, threshold
...
def model_f(X, w_h, w_h2, w_o, rank):
    w_h_mf, w_h2_mf, w_o_mf, c1, c2, c3, deg, t = fusion_update(w_h, w_h2, w_o, rank, 0.005)
    h = rectify(T.dot(X, w_h_mf))
    h2 = rectify(T.dot(h, w_h2_mf))
    py_x = softmax(T.dot(h2, w_o_mf))
    return h, h2, py_x, c1, c2, c3, deg, t
...
h_mf, h2_mf, py_x_mf, c1, c2, c3, deg, t = model_f(X, w_h, w_h2, w_o, rank)
...
for i in range(30):
    ...
    print "Threshold:", t
    print "Degree:", deg
    print "MF AUC:", auc.roc_auc(np.argmax(teY, axis=1), y_score[0])
